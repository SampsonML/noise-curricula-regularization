...
# Optimizers
# -----------------------------------------------------------------------------
def create_optimizer(name: str, learning_rate, weight_decay: float = 1e-4, rho: float = 0.05):
    ...
    elif name == 'SD': # Sign Descent
        return optax.chain(
            optax.add_decayed_weights(weight_decay),
            optax.trace(decay=0.9, nesterov=False),
            lambda updates, state: (jax.tree_map(jnp.sign, updates), state),
            optax.scale_by_schedule(lambda step: -learning_rate(step)) if callable(learning_rate) else optax.scale(-learning_rate)
        )
...
def cutout(image, mask_size=50):
    ...
    mask = tf.pad(mask, [[lower_h, height - upper_h], [lower_w, width - upper_w]], constant_values=0)
...
def get_schedule(schedule_type, init_lr, total_steps):
    if schedule_type == 'cosine':
        ...
        return optax.cosine_onecycle_schedule(total_steps, init_lr, pct_start=0.3, div_factor=20.0, final_div_factor=1e4)
    elif schedule_type == 'flat':
        return optax.constant_schedule(init_lr)
    elif schedule_type == 'decay':
        return optax.exponential_decay(init_lr, max(total_steps // 3, 1), 0.5, staircase=True)
    else:
...
def create_train_state(rng, args, total_steps, model):
    ...
    # Use our factory
    optimizer = create_optimizer(args.optimizer, schedule_fn, weight_decay=1e-4, rho=args.rho)
...
def main():
    ...
    parser.add_argument("--optimizer", type=str, default="SGD", choices=["Adam", "SGD", "SD", "SAM"])
    ...
    args = TrainingArgs(
        seed=pargs.seed, epochs=pargs.epochs, init_lr=pargs.init_lr,
        final_lr=pargs.final_lr, schedule=pargs.schedule, optimizer=pargs.optimizer,
        rho=pargs.rho, n_clean=pargs.n_clean, n_noisy=pargs.n_noisy,
        scale_c=pargs.scale_c, rise=pargs.rise, decay=pargs.decay,
    ...
    train_ds = tf.keras.utils.image_dataset_from_directory(train_dir, image_size=(256, 256), batch_size=None)
    
    # Pipeline: Augment -> Noise -> Normalize -> Batch
    train_ds = train_ds.map(augment, num_parallel_calls=AUTOTUNE)
    train_ds = train_ds.map(apply_noise_tf, num_parallel_calls=AUTOTUNE) # Uses dynamic NOISE_VAR
    train_ds = train_ds.map(normalize, num_parallel_calls=AUTOTUNE)
    train_ds = train_ds.batch(batch_size, drop_remainder=True)
    train_ds = train_ds.prefetch(AUTOTUNE)

    # Val/Test Pipeline
    val_ds_full = tf.keras.utils.image_dataset_from_directory(val_dir, image_size=(256, 256), batch_size=None)
    ...
    # Tracking
    history = {k: [] for k in ['loss', 'lr', 'val_loss', 'val_acc', 'test_loss', 'test_acc', 'noise']}
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_ds):
            ...
            if i % 1000 == 0:
                print(f"  Step {i}/{steps_per_epoch} | Loss: {metrics['loss']:.4f} | Acc: {metrics['accuracy']:.4f}")
            ...
            if global_step % save_freq == 0:
                ...
                def run_eval(ds):
                    ...
                    for b in ds:
                        ...
                        logits = state.apply_fn(
                            ...
                            jnp.array(b[0]), train=False, mutable=False
                        )
                ...
                history['loss'].append(np.mean(epoch_loss[-100:])) # last 100 steps mean
                ...
                with open(os.path.join(args.save_dir, f"{fname}_history.pkl"), "wb") as f:
                    pickle.dump(history, f)
                
                # Save Model
                with open(os.path.join(models_dir, f"{fname}_step_{global_step}.params"), "wb") as f:
