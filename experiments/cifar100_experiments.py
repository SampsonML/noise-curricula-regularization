...
class TrainingArgs:
    ...
    save_dir: str = "/scratch/gpfs/MELCHIOR/ms0821/lode_optimisers/production-code/lode-optimizer-v2/curriculum_learning/gradnorm-experiments/force-smooth/"
...
def process_sample(x: Dict[str, Any], is_training: bool, cfg: NoiseCfg) -> Dict[str, Any]:
    """
...
def create_dataset(dataset_builder, split: str, batch_size: int, is_training: bool, cfg: NoiseCfg, seed: int = 0):
    """
...
def create_optimizer(name: str, learning_rate: float, weight_decay: float, rho: float = 0.05) -> optax.GradientTransformation:
    """
    ...
    if name == 'Adam':
        ...
        return optax.chain(
            ...
            lambda updates, state: (jax.tree_map(jnp.sign, updates), state),
            optax.scale(-learning_rate) # Negative because we apply updates via addition
        )
...
def create_train_state(rng, learning_rate, total_steps, args: TrainingArgs):
    ...
    # Create Optimizer
    tx = create_optimizer(args.optimizer, learning_rate=1.0, weight_decay=args.weight_decay, rho=args.rho)
...
def get_schedule_fn(schedule_type, init_lr, total_steps):
    ...
    if schedule_type == 'cosine':
        ...
        return optax.cosine_onecycle_schedule(transition_steps=total_steps, peak_value=init_lr)
...
@jax.jit
def train_step(state, batch, dropout_rng, base_lr, prev_norm, scale_mask, increase_factor):
    """
    ...
    def loss_fn(params):
        ...
        loss = jnp.mean(optax.softmax_cross_entropy(logits, common_utils.onehot(batch['label'], 100)))
...
def run_training(args: TrainingArgs):
    ...
    val_iter = create_dataset(dataset_builder, 'train[90%:]', args.batch_size, False, NoiseCfg())
    test_iter = create_dataset(dataset_builder, 'test[:50%]', args.batch_size, False, NoiseCfg())
    # Noisy Test Iterator (uses args.scale_c as probability)
    test_iter_noise = create_dataset(dataset_builder, 'test[50%:]', args.batch_size, False, 
    ...
    scale_mask = 0.0 # initial scale mask (0: base learning rate, 1: scaled learning rate)

    print(f"Starting training for {args.epochs} epochs with optimizer {args.optimizer}...")
    for epoch in range(args.epochs):
        ...
        train_iter = create_dataset(dataset_builder, 'train[:90%]', args.batch_size, True, 
                                    NoiseCfg(sp_prob=noise_p), seed=args.seed + epoch)
        ...
        for step in range(train_steps):
            ...
            # Train Step
            state, metrics = train_step(state, batch, dropout_rng, base_lr, 
                                        baseline_norms[epoch], scale_mask, factor_vec[epoch])
            ...
            if step > 0 and step % (train_steps - 1) == 0:
                ...
                print(f"| Train Acc: {summary['accuracy']:.3f} Loss: {summary['loss']:.3f} ", end='')
        ...
        run_eval_phase(state, test_iter_noise, test_steps, 'noisy-test', history, newline=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-100 Training with Noise Injections and Custom Optimizers")
    
    # Training Config
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--epochs", type=int, default=100, help="Total epochs")
    parser.add_argument("--lr", type=float, default=0.02, help="Initial learning rate")
    parser.add_argument("--schedule", type=str, default="onecycle", help="LR schedule type")
    parser.add_argument("--optimizer", type=str, default="Adam", choices=["Adam", "SGD", "SD", "SAM"],
                        help="Optimizer choice: Adam, SGD, SD (Sign Descent), or SAM")
    parser.add_argument("--rho", type=float, default=0.05, help="Rho parameter for SAM optimizer (neighborhood size)")
    
    # Noise Injection Config
    parser.add_argument("--n_clean", type=int, default=5, help="Number of clean epochs between noise")
    parser.add_argument("--n_noisy", type=int, default=1, help="Number of noisy epochs per injection")
    parser.add_argument("--noise", type=float, default=0.0, help="Base noise level")
    parser.add_argument("--c", type=float, default=0.0, help="Noise probability/scale (sp_prob)")
    parser.add_argument("--decay", action='store_true', help="Use decaying noise schedule")
    parser.add_argument("--rise", action='store_true', help="Use rising noise schedule")
    parser.add_argument("--use_schedule", type=str, default="True", choices=["True", "False"],
                        help="Enable or disable noise schedule entirely")

    # Gradient/Adaptive Config
    parser.add_argument("--a", type=float, default=0.0, help="Shape parameter A (unused)")
    parser.add_argument("--b", type=float, default=0.0, help="Frequency parameter B (unused)")
    parser.add_argument("--p", type=float, default=0.0, help="LR increment param (unused)")
    parser.add_argument("--f", type=float, default=1.0, help="Gradient norm increase factor")
    parser.add_argument("--use_baseline", action='store_true', help="Use baseline gradient norms from file")
    ...
    args = TrainingArgs(
        seed=pargs.seed, n_clean_epochs=pargs.n_clean, epochs=pargs.epochs,
        n_noisy_epochs=pargs.n_noisy, lr=pargs.lr, schedule=pargs.schedule,
        optimizer=pargs.optimizer, rho=pargs.rho,
        noise_level=pargs.noise, shape_a=pargs.a, freq_b=pargs.b,
        scale_c=pargs.c, lr_inc_p=pargs.p, grad_inc_f=pargs.f,
        decay=pargs.decay, rise=pargs.rise, 
    ...
    filename_base = (f"cifar100_opt_{args.optimizer}_noise_{args.scale_c}_schedule_{args.use_schedule}_"
