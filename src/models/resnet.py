...
class _ModernResNetBase(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = ModernBody(self.channel_list, self.num_blocks_list, self.strides[1:], stage)(x)
...
class ResNet34(nn.Module):
    ...
    def __call__(self, x, train: bool = True):
        ...
        def make_layer(filters, blocks, stride):
            layers = []
            layers.append(StandardResidualBlock(filters, strides=(stride, stride), use_projection=True))
...
class ResNet50(nn.Module):
    ...
    def __call__(self, x, train: bool = True):
        ...
        def make_layer(filters, blocks, stride):
            layers = []
            layers.append(StandardBottleneckBlock(filters, strides=(stride, stride), use_projection=True))
            for _ in range(1, blocks):
